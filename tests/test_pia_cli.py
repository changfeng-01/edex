...
def test_pia_validate_cli_writes_aggregate_csv_and_report(tmp_path) -> None:
    ...
    assert (output_dir / "formal_validation_report.md").exists()
    assert (output_dir / "fairness_audit.csv").exists()
    assert (output_dir / "leakage_audit.csv").exists()
    assert (output_dir / "scenario_manifest.csv").exists()
    assert (output_dir / "method_registry.json").exists()
    assert (output_dir / "source_lock.json").exists()
