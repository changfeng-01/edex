...
- 不替代 ngspice、Spectre、HSPICE、Empyrean 或其他仿真器。
- 不自动宣称候选参数已经改善电路。
...
Artifact Store + Metadata Store + Job Runner
      |
CSV Import / ngspice / Empyrean / Future Adapters
```
...
- 队列深度；
- 最近失败任务；
- ngspice/PDK/adapter 可用性，但不得因“检测到路径”直接声明真实证据。
...
- 可选的 real ngspice/SKY130 测试由明确环境开关启用并 fail closed。
...
- mock/local fixture 的 `reportable_as_real_ngspice` 始终为 false。
