...
- Strategy benchmark: compares optimization strategies under shared budget and writes row-level results, per-strategy summary, strategy leaderboard, and report.
...
`not_evaluable` is separate from failed, skipped, and passed. Candidate generation is a next-run suggestion and must not be reported as completed optimization or physical validation. Mock ngspice runs are useful for software checks but are not reportable as real SKY130/ngspice evidence.
