...
def write_real_report(
    ...
    *,
    ...
    lines = [
        ...
        "- data_source = real_simulation_csv",
        "- engineering_validity = simulation_only",
        f"- expected_stage_count = `{summary.get('expected_stage_count')}`",
        f"- observed_stage_count = `{summary.get('observed_stage_count')}`",
        f"- output_coverage_ratio = `{summary.get('output_coverage_ratio')}`",
