...
当前版本处理的是仿真数据和外部仿真器输出文件，不是实物测试平台。所有真实 CSV 评价、上传 dashboard、product-demo、benchmark 和多智能体报告都必须保留：
...
这两个标签表示结果只能作为仿真分析、下一轮参数建议和软件链路验证依据。候选参数默认是 next-run simulation suggestions，必须重新仿真后才能讨论改善。外部仿真结果不能自动升级为芯片、样机、实验室实测、流片或已验证优化结论。
...
`goa-strategy-benchmark` compares candidate-generation strategies from existing GOA evidence without requiring a local simulator. It is a candidate-quality proxy benchmark, not a simulation-validation benchmark.
...
`multi-agent-run` is a local orchestration layer over existing evaluators, scorers, shared optimizers, external CSV artifacts, and registered circuit-domain agents. It does not replace the underlying algorithms.

```bash
python -m goa_eval.cli multi-agent-run \
  --task examples/tasks/instrumentation_amplifier_agent_task.yaml \
  --output-dir outputs/multi_agent_instrumentation_amplifier
```
...
```text
config/                           # evaluation, circuit profile, and parameter semantics configs
